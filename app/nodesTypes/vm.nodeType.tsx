    ...
    <BaseResourceNode id={id} type={type} selected={selected} data={data}>
      ...
      {data.status && (
        <div className="mt-2 flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full ${data.status === 'running' ? 'bg-green-400 animate-pulse' : 'bg-red-400'}`} />
          <span className="text-[10px] uppercase font-bold opacity-80">{data.status}</span>
        </div>
      )}
    </BaseResourceNode>
  );
